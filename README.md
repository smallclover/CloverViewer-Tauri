...
*   **右键菜单**：复制图片、复制路径、查看、编辑图片
*   **局域网分享**：右键图片即可生成临时链接和二维码，让同一网络内的设备预览或下载；可在设置中控制有效期和下载一次后失效
*   **旋转与翻转**：R 旋转、H/V 翻转
*   **图片编辑**：从工具栏、“编辑”菜单或右键菜单进入；可裁剪、旋转、标注（矩形、椭圆、箭头、画笔、马赛克、文字）、选择删除、撤销/重做，并导出 PNG / JPEG / WebP 或覆盖原图
...
*   **撤销/重做**：Ctrl+Z / Ctrl+Y
*   **导出**：Enter 复制到剪贴板 / 保存到桌面；工具栏可直接在查看器中打开普通截图
*   **局域网分享**：标注完成后可生成临时链接和二维码，供同一网络内的设备预览或下载截图
...
*   **设置面板**：分类 + 搜索的整页设置（常规 / 查看 / 截图 / 局域网分享 / 快捷键 / 缓存），每项带说明文字；含语言、主题、缩放灵敏度、截图 / 取色 / 滚动截图三个热键、放大镜、最小化到托盘、开机自启、软件更新及局域网分享规则
